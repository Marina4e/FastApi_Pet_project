...
def test_get_one_book(client, admin_test, get_token):
    ...
    res = client.get(f"/books/{book_id}", headers={"Authorization": f"Bearer {token}"})
...
def test_delete_book_admin(client, admin_test, get_token):
    ...
    res = client.delete(
        f"/books/{book_id}", headers={"Authorization": f"Bearer {token}"}
    )
...
def test_get_one_book_not_found(client, get_token, user_test):
    ...
    res = client.get("/books/9999", headers={"Authorization": f"Bearer {token}"})
