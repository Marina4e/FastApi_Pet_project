...
from app.auth.schemas import ArticleBase, ArticleOut, UserDetail, UserShort
...
def create_article(
    article: ArticleBase, db: Session = Depends(get_db), user=Depends(get_current_user)
):
    new_article = Article(
        title=article.title, content=article.content, owner_id=user.id
    )
...
def get_articles(db: Session = Depends(get_db), user=Depends(get_current_user)):
